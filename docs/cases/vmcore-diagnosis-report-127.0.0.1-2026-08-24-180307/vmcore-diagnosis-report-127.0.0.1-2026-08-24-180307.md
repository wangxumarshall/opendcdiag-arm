...
**根因判定：CPU 179（MPIDR 0x7A0300，Kunpeng-920 Taishan v110，Node 7）核内 L1d 数据通路存在间歇性软故障（最可能为 L1d 数据 SRAM 读出端口/位线感放/加载对齐逻辑失效）——属业界 "mercurial core" 类静默数据损坏（SDC）。建议立即下线该 CPU 并安排硬件维护（RMA），下线后可用本报告提供的长时 L1d 压测+spurious 监听方案验证根因。**
...
| **根因（子模块级）** | **CPU 179（MPIDR 0x7A0300，Kunpeng-920 Taishan v110，Node 7）核内 L1d 数据通路间歇性软故障**——最可能为 L1d 数据 SRAM 读出端口/位线感放/加载对齐逻辑的间歇失效；属业界所称 "mercurial core" 类静默数据损坏（SDC）。再细分到 SRAM 单元 vs 读出逻辑需芯片级诊断 |
...
**(C) 收窄到 L1d 数据 SRAM 或其读出端口/对齐逻辑。**
- 故障形态为**整字级损坏**（8/14 例全部 4 个半字、8/24 例 8 字节全部）与**半字混合**（8/14 例 read h0=真值 h1、read h3=真值 h0 精确命中，另两半字为损坏值）。这排除了 SRAM 单元的单/双比特翻转（那会是 1-2 bit 差异），而是**读出时字/半字的完整性破坏**——指向 SRAM 读端口、位线（bitline）感放、或加载对齐多路器（load-align mux）的间歇失效。
- 两种损坏形态不同（一次全垃圾、一次半字混合），说明不是固定逻辑门常坏，而是**受工况触发的间歇性软故障**——SRAM 单元在电压/温度边际下偶发读回错误，或读端口时序边际故障。
- 72 次 AT 复翻译"瞬时错、复测对"进一步指向**SRAM 间歇读回错误**（同一地址下次读即正确），而非 SRAM 单元的持久位损坏（那会每次都错）。
→ 缺陷最可能位于 **CPU 179 核内 L1d 数据 SRAM（或其读出端口/位线感放/对齐逻辑）的间歇性软故障**；MMU 页表遍历器因共享该读出路径而偶发假错。
