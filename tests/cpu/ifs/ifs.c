...
/*
 * In-Field Scan (IFS) is an Intel x86 hardware feature with no direct
 * equivalent on other architectures. On non-x86 platforms (e.g. ARM64)
 * the tests are kept as skip-stubs so they remain listed and schedulable,
 * reporting a clean skip rather than vanishing from the test catalog.
 *
 * This reserves the names for a future "In-Silicon Test" (IST) facility
 * providing equivalent at-field self-test coverage on those platforms.
 */
...
static int ist_skip_init(struct test *test)
{
    log_skip(CpuNotSupportedSkipCategory,
             "IFS is an Intel x86 hardware feature; not available on this "
             "architecture (future In-Silicon Test (IST) support reserved)");
    return EXIT_SKIP;
}

static int ist_skip_run(struct test *test, int cpu)
{
    __builtin_unreachable();
}
