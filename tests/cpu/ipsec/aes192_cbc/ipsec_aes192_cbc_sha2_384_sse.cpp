...
static void aes_hmac_sse_compute_golden(struct aes_hmac_sse_data *d) {
    ...
    hmac_sha384(d->hmac_key, HMAC_KEY_SIZE, d->golden_ciphertext, DATA_SIZE, d->golden_mac);
}
...
static int aes_hmac_sse_run(struct test *test, int cpu) {
    ...
    TEST_LOOP(test, 256) {
        ...
        memcmp_or_fail(ciphertext, d->golden_ciphertext, DATA_SIZE, "AES-192-CBC ciphertext mismatch (SSE)");

        aes_decrypt(d->aes_key, d->aes_iv, ciphertext, decrypted, DATA_SIZE);
        memcmp_or_fail(decrypted, d->plaintext, DATA_SIZE, "AES-192-CBC decryption mismatch (SSE)");

        hmac_sha384(d->hmac_key, HMAC_KEY_SIZE, ciphertext, DATA_SIZE, mac);
